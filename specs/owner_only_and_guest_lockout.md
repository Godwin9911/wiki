...
**Superseded in part** by `group_owner_only_cascading.md` (2026-08-03): the manager-bypass scope described below (`System Manager`/`Wiki Manager` bypassing Owner Only *enforcement*) was narrowed after a real-world test found a Wiki Manager account could still see Owner Only content it wasn't supposed to. Owner Only enforcement is now bypassed only by `Administrator`/the literal `Admin` role — see that doc for the corrected semantics and `_owner_only_bypass` in `wiki/permissions.py`.
...
- **Fixed 2026-08-04** (`fix/owner-only-sidebar-visibility`): the published sidebar tree (`build_nested_wiki_tree`/`get_public_wiki_tree` in `wiki_document.py`, rendered by `sidebar_tree.html`) pruned Owner Only nodes for *every* viewer unconditionally, with no `_owner_only_bypass`/owner check at all — so an Owner Only page never showed up in navigation even for Admin/Administrator or its own owner, even though they could still open it directly by route. Both functions now take an optional `user` and prune relative to that viewer; `get_public_wiki_tree` keeps the shared Redis-cached tree (fully pruned, `user=None`) as the fast path for everyone else, and only rebuilds uncached for a viewer who bypasses Owner Only or owns at least one Owner Only document. Toggle/badge visibility (still Admin-role-only) was left as-is — out of scope for this fix.
...
- **`Admin`** is added to the existing manager bypass (`MANAGER_ROLES` in `wiki/permissions.py`, alongside `System Manager`/`Wiki Manager`) — Admins see and can do everything, including toggling Owner Only. **(Superseded: see the note above — `System Manager`/`Wiki Manager` no longer bypass Owner Only enforcement specifically, only the general role-restricted-space bypass.)**
...
- **Field visibility** for `owner_only` uses Frappe's `permlevel` mechanism: the field is `permlevel: 1`, and only the `Admin` permission row is granted read/write at that level — hides it from Desk form view and the frontend for everyone else. Originally this was described as narrower than the enforcement bypass (`System Manager`/`Wiki Manager` could see the content but not the toggle); per the superseded-note above, the enforcement bypass is now equally narrow — both toggle visibility and enforcement bypass are `Administrator`/`Admin`-only.
